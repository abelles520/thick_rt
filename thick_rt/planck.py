...
def planck_freq(nu, T):
    ...
    """
    return (2*PLANCK_ERG*nu**3/SPEEDOFLIGHT**2)/(exp((PLANCK_ERG*nu)/(BOLTZMANN_ERG*T))-1)

def planck_wave(wave, T):
    """
    Planck law function in terms of wavelength
    """
    return (2*PLANCK_ERG*SPEEDOFLIGHT**2/wave**5)/(exp((PLANCK_ERG*SPEEDOFLIGHT)/(wave*BOLTZMANN_ERG*T))-1)
